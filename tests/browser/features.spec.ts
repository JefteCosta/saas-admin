...
test.group('Browser - CRUD Features', (group) => {
  ...
  async function seedAndLogin(page: any) {
    ...
    const saasMod = await Module.create({ slug: 'saas', name: 'SaaS', icon: 'Layers', position: 99 })
    const saasGrp = await FeatureGroup.create({ slug: 'admin-saas', name: 'Admin SaaS', moduleId: saasMod.id, position: 0 })
    await Feature.create({ slug: 'features.list', name: 'Features', icon: 'Layers', route: '/features', moduleId: saasMod.id, featureGroupId: saasGrp.id, position: 0, isMenuItem: true, isActive: true })

    const user = await User.create({ email: 'features@test.com', password: 'secret123', fullName: 'Features Test', roleId: role.id })
