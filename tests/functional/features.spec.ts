...
test.group('Features - Permissões por role', (group) => {
  ...
  async function getOrCreateFeatures() {
    const home = await Feature.firstOrCreate({ slug: 'home' }, { name: 'Home', icon: 'Home', route: '/', group: 'Plataforma', position: 0 })
    const profile = await Feature.firstOrCreate({ slug: 'profile' }, { name: 'Perfil', icon: 'User', route: '/profile', group: 'Plataforma', position: 1 })
    return { home, profile }
  }
...
test.group('Features - Team herda permissões', (group) => {
  ...
  test('member sem team não acessa feature que só existe no team', async ({ client, assert }) => {
    ...
    const home = await Feature.firstOrCreate({ slug: 'home' }, { name: 'Home', icon: 'Home', route: '/', group: 'Plataforma', position: 0 })
    await Feature.firstOrCreate({ slug: 'profile' }, { name: 'Perfil', icon: 'User', route: '/profile', group: 'Plataforma', position: 1 })
